...
const char *SqCarddata = "create table if not exists 'carddata' ( \
			...
			'Current' TEXT, \
			'Power'  varchar(2000) ); ";
const char *SqOther = "create table if not exists 'other' ( \
			'ID' INTEGER PRIMARY KEY AUTOINCREMENT, \
			'上传标识'	TEXT,	\
			'井名'  TEXT, \
            '类型'  TEXT, \
...
//		printf("[错误]sqlite3_open_v2失败,错误号=%d:%s\n", res, sqlite3_errmsg(pdb));
	...
	res = sqlite3_exec( pdb, SqCarddata, 0, 0, &errmsg );
	if(res != SQLITE_OK)
	{
//		char * tempinfo = NULL;
//		sprintf(tempinfo,"[错误]创建数据表失败：%d-%s \n", res, errmsg);
//		zlog_info(c, tempinfo);
		zlog_warn(c, "创建数据表carddata失败：%d-%s", res, errmsg);
//		printf("[错误]创建数据表失败：%d-%s \n", res, errmsg);
		sqlite3_free(errmsg);								// 释放内部分配的空间
		sqlite3_close(pdb);							// 来用关闭打开的数据库
		return 1;
	}
	res = sqlite3_exec( pdb, SqOther, 0, 0, &errmsg );
	if(res != SQLITE_OK)
	{
//		char * tempinfo = NULL;
//		sprintf(tempinfo,"[错误]创建数据表失败：%d-%s \n", res, errmsg);
//		zlog_info(c, tempinfo);
		zlog_warn(c, "创建数据表otherdata失败：%d-%s", res, errmsg);
//		printf("[错误]创建数据表失败：%d-%s \n", res, errmsg);
		sqlite3_free(errmsg);								// 释放内部分配的空间
		sqlite3_close(pdb);							// 来用关闭打开的数据库
		return 1;
...
//		printf("[错误]sqlite3_open_v2失败,错误号=%d:%s\n", res, sqlite3_errmsg(pdb));
	...
	bzero(timebuffer, 64);
	snprintf(timebuffer, 64, "%04d-%02d-%02d %02d:%02d:%02d", \
	...
	if(res){
		zlog_warn(c, "[错误]sqlite_exec失败,错误号=%d:%s\n", res, sqlite3_errmsg(pdb));
	}
...
int deviceInfoInsert(void *obj, int group, time_t test_time)
{
	...
	char *errmsg = NULL;
	time_t nowtime;
	device_base_information *pobj = (device_base_information *)obj;
	// 打开数据库
	res = sqlite3_open_v2(dbfile, &pdb, SQLITE_OPEN_READWRITE |SQLITE_OPEN_CREATE, NULL);
	if(res != SQLITE_OK)
	{
	...
	// 准备温度和实时数据
	float tempreture = modbusToFloat(pobj->work_temp);
	float real_data = modbusToFloat(pobj->realtime_data);
	// 插入基本数据insert into
	buffer = (char *)malloc(2048);
	bzero(buffer, 2048);
	snprintf(buffer, 2048, "insert into other (上传标识, 井名, 类型, 采集时间, 厂家代码, 仪表类型, 仪表组号, 仪表编号, 通信效率, 电池电压, 休眠时间, 仪表状态, 工作温度, 实时数据) \
			values (\'%s\',%d, %d, \'%s\', %d, %d, %d, %d, %d, %d, %d, %d, %f, %f);", \
			"NO", group, pobj->device_type, timebuffer,pobj->company_code, pobj->device_type, pobj->device_group, pobj->device_no, \
			pobj->comm_efficiency, pobj->bat_vol, pobj->sleep_time, pobj->device_sta, tempreture, real_data);

	res = sqlite3_exec(pdb, buffer, 0, 0, &errmsg);
	if(res){
		zlog_warn(c, "[错误]sqlite_exec失败,错误号=%d:%s", res, sqlite3_errmsg(pdb));
	}
//	// 读出最新序列,总序列数即为最后一个序列得数列号。
//    char** pResult;
//    int nRow;
...
int showRecordsByTime(time_t time)
{
	...
	bzero(sqlcode, 128);
	snprintf(sqlcode, 128, "selete * from carddata where CollectDateTime < \'%s\'", timebuffer);
...
//	sqlite3_stmt* pStmt = NULL;//sqlite3内部定义的一种"SQL语句
	...
	time_t work_tm = (now_time - OVERDAY * 86400);
	// 打开数据库
	res = sqlite3_open_v2(dbfile, &pdb, SQLITE_OPEN_READWRITE |SQLITE_OPEN_CREATE, NULL);
	if(res != SQLITE_OK)
	{
	...
	// 转换时间为字符串
	struct tm *now = localtime(&work_tm);
	char *timebuffer;
	timebuffer = (char *)malloc(64);
	bzero(timebuffer, 64);
	snprintf(timebuffer, 64, "%04d-%02d-%02d %02d:%02d:%02d", \
			now->tm_year + 1900, now->tm_mon + 1, now->tm_mday, \
//			now->tm_year + 1900, 7, 18,
			now->tm_hour, now->tm_min, now->tm_sec);
	sqlcode = (char *)malloc(128);
	bzero(sqlcode, 128);
	snprintf(sqlcode, 128, "delete from carddata where CollectDateTime < \'%s\'", timebuffer);

	res = sqlite3_exec(pdb, sqlcode, 0, 0, &errmsg);
	if(res != SQLITE_OK)
	{
		zlog_error(c, "删除过期数据库数据失败:%s",errmsg);
	}
//	else
//		zlog_info(c, "%s以前的数据被删除", timebuffer);
	free(timebuffer);
	free(sqlcode);
	// 关闭数据库
	sqlite3_free(errmsg);
...
int searchOldestData(void)
{
	...
	sqlcode = (char *)malloc(128);
	bzero(sqlcode, 128);
//	snprintf(sqlcode, 128, "select * from carddata");
	snprintf(sqlcode, 128, "select * from carddata where ID = (select min(ID) from carddata)");
...
//		printf("ID = %d\n",id);
//		char *name;
//		name = (char *)sqlite3_column_text(pStmt, 2);
//		printf("井名 = %s\n", name);
		char *tt;
		tt = (char *)sqlite3_column_text(pStmt,7);
//		printf("时间 = %s\n", tt);

		res = tmStringToFourUint16(tt, (unsigned short int *)&poilwell[0]->fuction_param.st_reserved.oldest_data_time);
		if(res != 0)
			zlog_error(c, "读取最早一条数据时间失败");
		res = sqlite3_step(pStmt);//读取某一条记录
   }
   sqlite3_finalize(pStmt);
   free(sqlcode);
