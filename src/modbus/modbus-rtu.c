...
#else
    return tcflush(ctx->s, TCIOFLUSH);				// tcflush:Unix终端I/O函数。作用：清空终端未完成的输入/输出请求及数据。
    																						// TCIOFLUSH:清除所有正在发生的IO数据,包括读数据和写数据
#endif
}
...
#else
    while ((s_rc = select(ctx->s+1, rfds, NULL, NULL, tv)) == -1) {		// 如果tv = NULL为阻塞：select将一直被阻塞
    																											// 直到某个文件描述符上发生了事件
    																											// select返回值0为超时,-1为出错
    																											// 如果为-1 在while循环中继续查询
                         ...
                         int stop_bit)
{
    ...
    ctx->backend_data = (modbus_rtu_t *) malloc(sizeof(modbus_rtu_t));		// backend_data 包含串口配置结构体
