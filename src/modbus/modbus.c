...
/*计算接收到功能码后的数据长度
 * 其长度为从功能码后开始包括地址+长度+字节数,不包括写入数据和CRC校验
 * 分为两种情况:1,MSG_INDICATION指示;2,MSG_CONFIRMATION确认
 * MSG_INDICATION为本机作为从机接收主机发来的指令
 * MSG_CONFIRMATION为本机作为主机接收到从机发来的确认指令
 * */
                                                  ...
                                                  msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION) {							//指示模式为本机作为从机接收主机发来的指令
        if ((function <= _FC_WRITE_SINGLE_REGISTER) ||
        			(function == _FC_DIAGNOSTICS)){							//wsf20150113
            length = 4;
        } else if (function == _FC_WRITE_MULTIPLE_COILS ||
                   function == _FC_WRITE_MULTIPLE_REGISTERS) {		//0x10写多个寄存器,起始地址高低+寄存器数量高低+字节数 = 5
            length = 5;
        ...
        switch (function) {
        case _FC_WRITE_SINGLE_COIL:								//0x05写单个线圈起始地址高低+输出数量高低共4个字节
        case _FC_WRITE_SINGLE_REGISTER:						//0x06写单个寄存器,起始地址高低+寄存器数量高低共4个字节
        case _FC_WRITE_MULTIPLE_COILS:						//0x0F写多个线圈,起始地址高低+输出值高低共4个字节
        case _FC_WRITE_MULTIPLE_REGISTERS:				//0x10写多个寄存器,起始地址高低+寄存器数量高低共4个字节
        case _FC_DIAGNOSTICS:											//0x08诊断命令 wsf20150114
            length = 4;
            break;
        default:
            length = 1;
...
static int compute_data_length_after_meta(modbus_t *ctx, uint8_t *msg,
                                          msg_type_t msg_type)
{
    int function = msg[ctx->backend->header_length];					// 从接收数组msg中提取function功能码
    int length;
    /*
 ...
 */
{
    ...
    if (ctx->debug) {						// 如果是debug模式显示当前状态
        if (msg_type == MSG_INDICATION) {
            printf(">[提示]Waiting for a indication...\n");
        } else {
     ...
     * information. */
    step = _STEP_FUNCTION;
    length_to_read = ctx->backend->header_length + 1;		//header_length初始值为1,即lenth_to_read当前为2
...
static int receive_msg_net(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
/*
 * 指针msg用于保存接收到的数据首地址
 */
{
    ...
    if (ctx->debug) {						// 如果是debug模式显示当前状态
        if (msg_type == MSG_INDICATION) {
            printf(">[提示]Net1000等待一个命令...\n");
        } else {
            printf(">[提示]Net1000等待一个确认...\n");
        }
    }
...
static int receive_msg_comm(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
/*
 * 指针msg用于保存接收到的数据首地址
 */
{
...
int modbus_receive(modbus_t *ctx, uint8_t *req, int flag)
{
	switch(flag)
	{
		case 11:
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    uint8_t rsp[MAX_MESSAGE_LENGTH];				// 申请长度为260的用于返回数据的数组rsp
...
//        	 if (address <= mb_mapping->nb_registers) {
        	 ...
        	 * 中增加对0x06功能码的支持
        	 */
            mbWriteSigleRegister(address, data);
...
int modbus_test_debug(modbus_t *ctx)
{
	if(ctx->debug)
		return 1;
	else
